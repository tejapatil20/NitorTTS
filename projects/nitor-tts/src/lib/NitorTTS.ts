export class  NitorTTS {
  ...
  constructor() {
   ...
   init(voice:any,lang:any,volume?:number,rate?:number,pitch?:number){
    this.voices=window.speechSynthesis.getVoices();
    if(this.voices.filter(function(v:SpeechSynthesisVoice) { return v.name == voice})[0]){
      this.speech.voice=this.voices.filter(function(v:SpeechSynthesisVoice) { return v.name == voice})[0];
    }
